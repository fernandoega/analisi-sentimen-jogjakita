...
def preprocess(text):
    """Preprocess text: lowercase, remove special chars, stemming, stopword removal"""
    text = text.lower()
...
def get_sentiment_color(sentiment):
    ...
    return COLORS["success"] if sentiment == "Positif" else COLORS["error"]
...
def get_confidence_score(history_item):
    """Get confidence score from history item"""
    return history_item["Probabilitas Positif (%)"] if history_item["Sentimen"] == "Positif" else history_item["Probabilitas Negatif (%)"]
...
def create_bar_chart():
    """Create sentiment trend bar chart"""
    ...
    # Count positive/negative per 5 predictions
...
def create_wordcloud(sentiment_type="all"):
    ...
    if sentiment_type == "positive":
        ...
        color_func = lambda *args, **kwargs: COLORS["success"]
    elif sentiment_type == "negative":
        reviews = [h["Ulasan"] for h in history if h["Sentimen"] == "Negatif"]
        color_func = lambda *args, **kwargs: COLORS["error"]
    else:
        reviews = [h["Ulasan"] for h in history]
        color_func = lambda *args, **kwargs: COLORS["primary"]
...
with st.sidebar:
    ...
    with col1:
        st.write("🌙 Mode Gelap")
    with col2:
        st.toggle("", key="dark_mode_toggle")

    # Update dark mode state
    if st.session_state.dark_mode != st.session_state.dark_mode_toggle:
...
with col2:
    ...
    <div class="stats-card">
        <div style="font-size: 2rem; margin-bottom: 8px;">💚</div>
        <div class="stats-value" style="color: {COLORS['success']};">{positive}</div>
...
with col3:
    ...
    <div class="stats-card">
        ...
        <div class="stats-value" style="color: {COLORS['error']};">{negative}</div>
